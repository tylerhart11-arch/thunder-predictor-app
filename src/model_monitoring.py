...
def feature_schema_hash(feature_cols: list[str]) -> str:
    normalized = [str(col) for col in feature_cols]
    return str(hash(tuple(normalized)))
...
def build_monitoring_artifacts(
    ...
    if completed.empty:
        ...
        return MonitoringArtifacts(summary=summary, windows=empty_df, segments=empty_df, confidence_buckets=empty_df, completed_outcomes=empty_df)
    ...
    windows = pd.concat(
        [
            _window_metrics(scope="league", frame=league, target_col="ACTUAL_HOME_WIN", prob_col="PRED_HOME_WIN_PROB", windows=[10, 25, recent_window, 50]),
            _window_metrics(scope="thunder", frame=thunder, target_col="THUNDER_ACTUAL_WIN", prob_col="THUNDER_WIN_PROB", windows=[5, thunder_recent_window, 20]),
        ],
        ignore_index=True,
    )
    segments = pd.concat(
